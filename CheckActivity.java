package com.skyfoxstudio.everydaycheck.ui;
...
import com.skyfoxstudio.everydaycheck.R;
import com.skyfoxstudio.everydaycheck.core.service.ServiceAutoCheck;
import com.skyfoxstudio.everydaycheck.util.LoggerUtil;
import com.skyfoxstudio.everydaycheck.util.pref;
...
public class CheckActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String jsonData = pref.getStringData(getApplicationContext(), "profileData", "");
        if(jsonData.isEmpty()){
            writeLog("Authentication info is not set.");
            Toast.makeText(this, "인증 정보가 설정되어있지 않습니다.", Toast.LENGTH_SHORT).show();
        } else {
            try {
                writeLog("Loading authentication info...");
                JSONObject jsonObject = new JSONObject(jsonData);
                encryptProfile(jsonObject.getString("name"),
                        jsonObject.getString("birth"), jsonObject.getString("password"),
                        jsonObject.getString("orgCode"), jsonObject.getString("region"));
            } catch (JSONException e) {
                writeLog("Failed load! authentication info : JSONException");
                e.printStackTrace();
            }
        }
    }
    ...
    private void writeLog(String msg){
        LoggerUtil.writeLog(getApplicationContext(), LogTag + msg);
    }
    ...
    private void initializeUI(){
        writeLog(getApplicationContext(), "수동 자가진단 시작 준비중");
    }
    ...
    private void writeLog(Context context, String message){
        ...
        String NowLog = sdf.format(date) + "\n" + message +"\n";
        String PreviousLog = pref.getStringData(context, "logFile", "");
        pref.setStringData(context, "logFile", NowLog + "\n" + PreviousLog);
    }
    @Override
    protected void onPause() {
        super.onPause();
        overridePendingTransition(
                R.anim.activity_out1,
                R.anim.activity_out2
        );
    }
}